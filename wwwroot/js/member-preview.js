...
function AddDashboard(i, e) {
    //var i = jq('#trackid').val();
...
function validateform(arg) {
    if (arg == 'next') {
        ...
        // debugger;
        var pcontact = jQuery('#inpContact').filter(':selected').val();
...
function PayData(ProjId) {
    ...
    jQuery.ajax({
        ...
        data: { 'model': model, 'pathChK': pathChK, 'ContPath': 'Member', 'ActPath': 'Preview', 'SuccessPath': 'SaveCopyCenterInfo' },
        async: false,
        success: function (response) {
            if (response.Status = 'success') {
...
jQuery(document).ready(function () {
    jQuery('.colpSolrPage').click(function () {
        // Toggle the 'solr' class on the closest 'ul'
        jQuery(this).closest('.pageResult').find('ul').toggleClass('solr');

        // Toggle  icons
        jQuery(this).toggleClass('fa-minus-square-o fa-plus-square-o');
    });
    jQuery('.colpAllPage').click(function () {
        jQuery(this).toggleClass('fa-plus fa-minus');
        var parentdiv = jQuery(this).parent();
        parentdiv.next('.entireWrap').toggleClass('solr');
        //jQuery('.pageResult ul').toggleClass('solr')
    });
});
