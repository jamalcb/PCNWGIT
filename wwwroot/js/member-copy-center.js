...
function OpenPrintModel() {
    ...
    jq.ajax({
        ...
        success: function (response) {
            jQuery('#inpCompany').val(response.Company);
            jQuery('#hdninpCompany').val(response.Id);
            jQuery('#Uid').val(response.ConID);
            jQuery('#inpContact').val(response.Name);
            jQuery('#inpAddr').val(response.Address);
            jQuery('#inpCity').val(response.City);
            jQuery('#inpState').val(response.StateCode);
            jQuery('#inpZip').val(response.Zip);
            jQuery('#inpPhone').val(response.Phone);
            jQuery('#inpEmail').val(response.Email);
        },
...
function PayData() {
    ...
    jQuery.ajax({
        ...
        data: { 'model': model, 'pathChK': pathChK, 'ContPath': 'Member', 'ActPath': 'CopyCenter', 'SuccessPath': 'SaveCopyCenterInfo' },
        async: false,
        success: function (response) {
...
function GetCopyCenterPriceDetail() {
    jq.ajax({
        ...
        dataType: 'json',
        //contentType: "application/json; charset=utf-8",
        url: '/Member/GetCopyCenterPriceDetail/',
        data: {},
        async: false,
        success: function (response) {
...
function ShowModalC(arg) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            jQuery('#RviewDocFr').attr('href', '/Member/ViewDoc?OrderId=' + item.OrderId);
...
function ShowModalD(arg) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            var totalPrice = 0;
            var item = response.data;
            jQuery('#COviewDocFr').attr('href', '/Member/ViewDoc?OrderId=' + item.OrderId);
            jQuery.each(response.data.GetTblProjs, function (index, item) {
...
function ShowModal(arg) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            var totalPrice = 0;
            var item = response.data;
            jQuery('#viewDocFr').attr('href', '/Member/ViewDoc?OrderId=' + item.OrderId);
            jQuery.each(response.data.GetTblProjs, function (index, item) {
                rows += '<tr><td>' + item.FileName + '</td><td class="">' + item.Pages + '</td><td class="">' + item.Copies + '</td><td class="">' + (item.PrintName != null ? item.PrintName : "B & W") + '</td><td class="">' + item.Size + '</td><td class="">' + item.Price + '</td></tr>';
                var price = parseFloat(item.Price);
                // Add the price to the total
...
function validateform(arg) {
    if (arg == 'next') {
        ...
        var pcontact = jQuery('#inpContact').val();
        jQuery('#inpContact').next('span').html('');
        if (pcontact == undefined || pcontact == '' || pcontact == null) {
            jQuery('#inpContact').next('span').html('Please enter contact.');
            isValide = false;
        }
...
function ChangePayOrOrderButton() {
    var selectedvalue = jQuery("#inpPayInfo").val();
    var iconText = document.getElementById("SubmitId").getElementsByTagName("i")[0].outerHTML;

    if (selectedvalue == 2) {
        // jQuery to change the text content of the <span> element
        jQuery("#SubmitId").html(iconText + " Order");
    }
    else {
        // jQuery to change the text content of the <span> element
        jQuery("#SubmitId").html(iconText + " Pay");
    }
}
