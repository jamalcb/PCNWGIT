...
function SaveDirectory() {
	jq('#loader-overlay2').css('display', 'block');
	...
	jq.ajax({
		...
		dataType: 'json',
		//contentType: "application/json; charset=utf-8",
		url: '/StaffAccount/SaveDirectory/',
		data: { 'model': model },
		async: true,
		success: function (response) {
			//alert(response.statusMessage);
...
jq(document).ready(function () {
	...
	const sections = [
		{ checkbox: '#chkBillAddress', preview: '.billaddrPreview' },
		{ checkbox: '#chkLogo', preview: '.logoPreview' },
		{ checkbox: '#chkCompany', preview: '.companyPreview' },
		{ checkbox: '#chkDBA', preview: '.dbaPreview' },
		{ checkbox: '#chkPC', preview: '.primarycontPreview' },
		{ checkbox: '#chkMailAddress', preview: '.mailaddrPreview' },
		{ checkbox: '#chkPB', preview: '.businessPreview' },
		{ checkbox: '#chkPhone', preview: '.phonePreview' },
		{ checkbox: '#chkEmail', preview: '.emailPreview' },
		{ checkbox: '#chkSpeciality', preview: '.specPreview' },
		{ checkbox: '#chkLicense', preview: '.licensePreview' },
	];

	sections.forEach(section => {
		const checkbox = jq(section.checkbox);
		const preview = jq(section.preview);

		updatePreview(checkbox, preview);
	...
	function updatePreview(checkbox, preview) {
		if (!checkbox.prop('checked')) {
			preview.addClass('hidepreview_class');
		} else {
			preview.removeClass('hidepreview_class');
		}
	}
...
jQuery(document).ready(function () {
	ShowPayment();
	SelectReason();
	var gracevalu = jQuery("#Grace").val();
	if (gracevalu != null && gracevalu > 0) {
		jQuery("#TxtGrace").val(gracevalu);
	}
...
function validateform() {
	...
	//if (dba == undefined || dba == '' || dba == null) {
	//    jQuery("#Dba").addClass('error')
	//    isValide = false;
	//}
...
function UpdateDirectory() {
	...
	jq.ajax({
		type: "POST",
		dataType: 'json',
		url: '/StaffAccount/UpdateDirectory/',
		data: { 'model': model },
		async: true,
		success: function (response) {
			if (response.success) {
...
function MemberUserAdmin(ID, Email) {
	...
	jq.ajax({
		type: "POST",
		dataType: 'json',
		url: '/StaffAccount/AdminUserContact/',
		data: { 'model': model },
		async: true,
		success: function (response) {
			if (response.success) {
...
function MemberUserDailyReport(ID, Email, ctrl) {
	...
	jq.ajax({
		type: "POST",
		dataType: 'json',
		url: '/StaffAccount/MemberUserDailyReport/',
		data: { 'model': model },
		async: true,
		success: function (response) {
			if (response.success) {
...
function GetPacificCardDetails() {
	jQuery.ajax({
		type: "GET",
		dataType: 'json',
		url: '/StaffAccount/GetPacificCardDetails',
		data: {},
		async: true,
		success: function (response) {
...
function GoToReg(ctrl) {
	...
	var url = "/StaffAccount/RenewalPayment" +
		"?PkgText=" + pkgMemText +
...
function DeleteUserManagement(Id, ctrl) {
	if (confirm("Are you sure you want to delete ?")) {
		jQuery.ajax({
			type: "POST",
			dataType: 'json',
			url: '/StaffAccount/DeleteUserManagement/',
			data: { 'Id': Id },
			async: true,
			success: function (response) {
				alert(response.statusMessage);
...
function CopyGraceValue() {
	var gracevalue = jQuery("#TxtGrace").val();
	jQuery("#Grace").val(gracevalue);
}
function UpdateGracePeriod() {
	var GracVal = jQuery("#Grace").val();
	jQuery("#TxtGrace").next('span').html('');
	if (GracVal == null || GracVal == "" || GracVal == undefined) {
		jQuery("#TxtGrace").next('span').html('Please insert grace period.');
		return false;
	}
	var model = {};
	model.Id = jQuery('#hdnId').val();
	var renewalDate = new Date(jQuery('#Gracedate').val());

	if (isNaN(renewalDate.getTime())) {
		console.error("Invalid date:", renewalDate);
	} else {
		console.log("Parsed date:", renewalDate);
	}

	// Manually format the date to 'yyyy-MM-ddTHH:mm:ss' without converting to UTC
	var formattedDate = renewalDate.getFullYear() + '-' +
		('0' + (renewalDate.getMonth() + 1)).slice(-2) + '-' +
		('0' + renewalDate.getDate()).slice(-2) + 'T' +
		('0' + renewalDate.getHours()).slice(-2) + ':' +
		('0' + renewalDate.getMinutes()).slice(-2) + ':' +
		('0' + renewalDate.getSeconds()).slice(-2);

	console.log("Formatted date:", formattedDate);

	model.RenewalDate = formattedDate; 

	model.Grace = jQuery('#Grace').val();
	model.Email = jQuery("#Email").val();
	jQuery.ajax({
		type: "Post",
		dataType: 'json',
		url: '/ChargeBee/UpdateGracePeriod',
		data: { 'model': model },
		async: true,
		success: function (response) {
			if (response.Status=='success') {
